...
**Incidental finding, corrected in-session, not treated as a blocker**: the installer's `[network] source = "from-dhcp"` had baked a **static, IPv6-only** configuration into `/etc/network/interfaces` (`fec0::.../64`, a non-routable site-local prefix, no IPv4 address at all) — meaning the installed system had no way to reach the real internet at all until an IPv4 address and default route were added manually for this test session (`ip addr add 10.0.2.15/24 dev vmbr0 ; ip route add default via 10.0.2.2`, not persisted to any config file). **Scope correction**: this is documented here as an **observed behavior of this specific QEMU/SLIRP/`proxmox-auto-install-assistant` combination, reproduced consistently across multiple installs in this session** — it is not evidence about how Proxmox VE's `from-dhcp` handling behaves on real hardware against a real DHCP server, and must not be generalized into a claim about Proxmox installations generally. It's flagged as something Milestone 1 should specifically check for under a real DHCP server before assuming either outcome, not as a predicted or likely real-hardware defect. Not investigated further here (out of this investigation's scope).
...
- The IPv6-only DHCP-capture finding is worth tracking as a Milestone 1 verification item — directly relevant to whether first-boot package installation (this investigation's whole subject) can reach a package repository — but it stays scoped to what was actually observed (this QEMU/SLIRP/installer combination) until checked against a real DHCP server on real hardware, per the correction above.
