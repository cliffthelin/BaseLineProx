...
Status: **a confirmed real-identifier finding exists in current content and Git history; current content is corrected, history is not; the broader privacy verification gate remains pending operator action.**

```
identity_scan.current_content: known_finding_pending_correction
identity_scan.git_history:     known_finding
identity_scan.full_scan:       pending_operator_verification
development_blocked:           false
physical_P0_closeout_blocked:  true
```

**Confirmed finding (2026-09-24):** an earlier revision of `docs/design/testpersistence-prd.md` carried a real personal name in its `Owner:` field - outside the repository-account-metadata exception, and a genuine prohibited-identity match. The field has been removed from current content in a normal follow-up commit (no value repeated anywhere in the fix). It remains present in prior Git history (the commit that introduced it, and any commit before this fix that touched the file), which a `current_tracked`-only scan does not reach - only a `git_history`-scope scan or manual inspection does. `identity_scan.current_content` is therefore `known_finding_pending_correction` (the correcting commit exists but hasn't itself been verified clean yet) rather than `clean`, and `identity_scan.git_history` stays `known_finding` until that history is addressed. **Removing the value from history requires rewriting the affected commit(s) and force-pushing** - a destructive operation on a shared branch that is never performed without the operator's own explicit, separate authorization; it has not been requested or performed. See "TestPersistence PRD correction pass (2026-09-24)" below for the exact non-destructive and history-rewrite options.
...
**The remaining action before the first P0 capture is for the operator to run the interactive scanner locally** (`python3 tools/interactive_denylist_scan.py`) and type the three real protected values at its hidden prompts - Claude never sees them, never asks for them in chat, and the mechanism is designed so they cannot reach any log, transcript, file, or fixture. This has **not** been run by any session as of this writing - `identity_scan.full_scan` stays `pending_operator_verification`, never `passed`, until an operator runs it and reports a clean result back (and, separately, until the git-history finding above is resolved one way or another). A `findings` or `scan_incomplete` result reopens the privacy gate regardless of anything else in this document; it does not invalidate unrelated architecture or planning work done in the meantime. Blocked by the pending gate: final P0 privacy closeout, physical Phase P1, production identity enrollment, and publication of any retained evidence as privacy-clean. Not blocked: documentation, schema/unit-test-only design work, and synthetic (no real hardware, no real identity) experimentation - see [testpersistence-prd.md](testpersistence-prd.md) for the current instance of that.

## TestPersistence PRD correction pass (2026-09-24)

The `Owner:` field finding above was found and fixed in the same pass as a set of architectural corrections to `testpersistence-prd.md` (logical vs. carrier identity naming, manifest/ledger authenticity, honest rollback-authority limits, scoped clone-detection claims, automatic grant suspension on disable/uninstall, isolated untrusted-content inspection, a fixed unknown-device state contradiction, domain-separated test/production manifest authority replacing a mutable boolean, ownership separated from application grants, a corrected acceptance case 20, a clarified acceptance case 4, and reserved recovery-ledger capacity for full-store handling). The corrected file has been committed normally (a new commit; not a history rewrite).

**Non-destructive option (default, used here):** leave prior history as-is; current content is fixed going forward; `identity_scan.current_content` moves to `known_finding_pending_correction` and then to `clean` once a `current_tracked`-scope scan (not `git_history`) reports clean; `identity_scan.git_history` stays `known_finding` indefinitely, documented here rather than hidden.

**History-rewrite option (destructive, requires explicit operator authorization before execution):** `git filter-repo` (or `git filter-branch`/BFG) to strip the value from the affected commit(s), rewriting `1f10ae2` (and any later commits built on it) to new hashes, then `git push --force-with-lease origin main`. This rewrites published history other clones may already have fetched, is irreversible on the remote once other clients re-sync, and is exactly the class of action this project's standing rules require explicit authorization for before performing - it has not been authorized and has not been performed.
