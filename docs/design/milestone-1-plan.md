...
**Gate C: partial, boundary 3 promoted but not yet passing (2026-09-22).** `baseline/lib/drive_setup_install.py` is written and unit-tested (14 new tests, 129/129 total): QEMU invocation construction (sparse-file-only, `guestfwd`-isolated networking, `order=d,once=d` for install / `order=c` with no CD-ROM attached for post-install boot per decision record 04's fix), bounded process waiting with process-group kill on timeout, static (`fdisk`/`blkid`-only, never mount/loop-device) image verification, and streamed forbidden-byte scanning. This pass real-verified the module's own automatable mechanisms for the first time: a real QEMU launch via `RealInstallRunner.popen`, and a real HMP monitor-socket client for `send_monitor_command` (previously a `NotImplementedError` stub) used for 13 real `screendump` captures against a live QEMU process. **Gate C's actual stated requirement — the literal `Finished: 'ok'` completion text captured via screendump — was not re-completed this pass**, honestly reported rather than inferred from QEMU exit code or partition structure: the interactive Terminal UI installer's email-address field could not be completed due to VNC-keystroke keyboard-layout non-determinism (`shift-2` intermittently producing `"` instead of `@` across otherwise-identical QEMU boots), a test-harness-tooling issue (`vnc_type.py`, PRD §6, never shipped) unrelated to the promoted module itself. See [decision-records/20-boundary3-install-module-partial.md](decision-records/20-boundary3-install-module-partial.md). Next attempt should either retry on a fresh boot (this keyboard-layout variance may itself be non-deterministic per-boot, as interface naming already showed) or wait for decision record 16's guestfwd blocker to be resolved so the fully-automated, keystroke-free install path can be used instead.
