...
Status: **One real bug found and fixed (`ifreload --syntax-check`'s interactive-vs-non-interactive exit-code mismatch); a second, deeper, unresolved design-level problem found immediately after — the additive dual-stanza approach itself does not work.** See [decision-records/12-gateA-additive-repair.md](decision-records/12-gateA-additive-repair.md), [13-syntax-check-advisory-fix.md](decision-records/13-syntax-check-advisory-fix.md), and [14-additive-dhcp-apply-gap.md](decision-records/14-additive-dhcp-apply-gap.md). 78/78 unit tests passing. Direct evidence (a full verbose `ifup -v vmbr0` trace, zero DHCP-related activity anywhere in it) shows ifupdown2's real bring-up path processes only the first `iface vmbr0 ...` stanza it finds and never acknowledges a second one exists — passing `--syntax-check` was never evidence this would work, since that validates each stanza independently. **This is now a design-level problem for the additive repair, not a bug to patch** — see record 14's recommended next steps (source review, a cleaner reordering test, or a different mechanism entirely for adding DHCP to an interface that already has a static stanza for another family). **No push to `cliffthelin/baseline` at any point.** Gates B–F remain unstarted.
