...
Status: **Architecture revised (v3) after discovering an existing, substantially complete host-network repair implementation on `cliffthelin/baseline` (`repair/host-network-reset-to-dhcp`, commits `0368d7a`/`8509af6`, draft PR #1, 34/34 tests passing).** Phase 0's `networking_unresolved` result (Gate A, v2 wording) is superseded, not overturned — the underlying broken-static-config finding is real and stands (see decision record 10), but the correct response is to detect it as a first-boot repair case using already-built machinery, not to keep treating it as an installer-level blocker. See [decision-records/11-repair-branch-comparison.md](decision-records/11-repair-branch-comparison.md) for the full read-only comparison this revision is based on. **No code has been copied from `cliffthelin/baseline`, no push or modification was made to it, and no new QEMU installation has been run under this revision.** Gate A is redefined below (§5) and has not yet been attempted under its new definition.
