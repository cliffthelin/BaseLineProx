...
Status: **Decision record 14's design-level gap is now root-caused and fixed (a supplementary `dhclient <target>` call after apply, since ifupdown2's real bring-up only ever dispatches the first `iface <name>` stanza it finds for a given interface name — confirmed directly via ifupdown2 source review and `ifquery`). Real QEMU re-verification confirms DHCP address acquisition now works for the additive path. A second, distinct, environment-specific gap was found immediately after: the acquired lease carries no default route, root-caused to QEMU/SLIRP's built-in DHCP server not sending a `routers` option even when requested (confirmed via a fully clean DISCOVER/OFFER/REQUEST/ACK cycle and `dhclient.conf` inspection) — not a code defect, and not fixed with a guessed-gateway workaround in production code since that would be incorrect on real networks where the DHCP server and gateway differ.** See [decision-records/12-gateA-additive-repair.md](decision-records/12-gateA-additive-repair.md), [13-syntax-check-advisory-fix.md](decision-records/13-syntax-check-advisory-fix.md), [14-additive-dhcp-apply-gap.md](decision-records/14-additive-dhcp-apply-gap.md), and [15-supplementary-dhclient-fix-and-slirp-route-gap.md](decision-records/15-supplementary-dhclient-fix-and-slirp-route-gap.md). 81/81 unit tests passing. **Gate A still does not achieve a full real-QEMU pass** (address + route + gateway + DNS + HTTPS) — the pipeline correctly fails closed and rolls back on the missing-route condition rather than reporting false success, but that is not the same as passing. **A further, more fundamental infrastructure problem was then found: this host can no longer build a fresh disposable Proxmox install at all** — four independent attempts (with and without `restrict=on`, under both heavy and normal host load) all failed at the guestfwd-dependent answer-file-fetch step; direct in-guest testing (`openssl s_client`) confirms TCP connectivity to the guestfwd target hangs for 100+ seconds with zero response, while the answer server itself responds correctly and instantly when queried directly from the host. Root cause not found (packet capture is blocked by missing `CAP_NET_RAW`; a host package downgrade or reboot were identified as the most likely next diagnostic steps but were not taken, as both require the user's own decision). This is a host/environment issue, not a code defect, and does not affect decision record 15's own DHCP-router-option finding (which used an already-installed guest with no guestfwd involved) — but it does mean that finding's one open question (whether removing `restrict=on` restores the DHCP `routers` option) remains untestable until this is resolved. See [16-guestfwd-install-path-broken.md](decision-records/16-guestfwd-install-path-broken.md). **No push to `cliffthelin/baseline` at any point.** Gates B–F remain unstarted, and no new disposable Proxmox install can currently be built on this host at all.
