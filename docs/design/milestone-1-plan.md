...
Status: **Gate A implementation attempted under its redefined (v3) wording — does not yet fully pass.** See [decision-records/12-gateA-additive-repair.md](decision-records/12-gateA-additive-repair.md). The additive repair pipeline (preserving the actual reproduced Fixture A, no `ipv6=off`) is implemented, imported/rebased into this repo additively (`boot/provision.sh`'s existing `handoff.py`/wizard deployment untouched), and passes 71/71 unit tests. One disposable QEMU integration run (per instruction) demonstrated the pipeline's fail-closed safety directly against real infrastructure — correct exact-diff proposal, correct backup/rollback-arming, and a correct refuse-and-restore when `ifreload --syntax-check` flagged a real, pre-existing `bridge-fd 0` value — but did not reach a successful apply/verify, so full Gate A success (working DHCP, verified, persisting across reboot) was not achieved. See decision record 12 for the exact blocker and retained limitations. **No push to `cliffthelin/baseline` at any point.** Gates B–F remain unstarted.
