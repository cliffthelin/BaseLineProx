...
Status: **One real bug found and fixed (`ifreload --syntax-check`'s interactive-vs-non-interactive exit-code mismatch — confirmed against real infrastructure, affects the whole application, not just this feature); Gate A still does not fully pass.** See [decision-records/12-gateA-additive-repair.md](decision-records/12-gateA-additive-repair.md) and [decision-records/13-syntax-check-advisory-fix.md](decision-records/13-syntax-check-advisory-fix.md). 78/78 unit tests passing. The syntax-check refusal that blocked record 12's Gate A attempt is fixed and verified gone in a fresh QEMU run; a second, deeper, genuinely unresolved problem was then found: after a successful `ifreload -a` apply, the additive `inet dhcp` stanza does not actually acquire an IPv4 address (confirmed via both `ifreload -a` and a full `ifdown`/`ifup` cycle) — the dual-stanza-per-interface-name approach itself may not be how this ifupdown2 version actually brings up a second address family at apply time, as opposed to validating it at syntax-check time. Not fixed this pass. **No push to `cliffthelin/baseline` at any point.** Gates B–F remain unstarted.
