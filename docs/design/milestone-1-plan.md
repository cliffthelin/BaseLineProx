...
Status: **First genuine, independently-verified Gate A apply-time pass (2026-09-22).** Routed around the still-unresolved guestfwd blocker (decision record 16) by using the interactive Terminal UI installer (proven reliable for last night's real `/dev/sdc` migration) instead of the automated/answer-file path, then staging Fixture A's exact byte content directly onto the fresh install. The additive repair pipeline, run for real against genuine ifupdown2/dhclient/SLIRP, achieved a full pass: address, route, gateway, DNS, and HTTPS all independently re-verified afterward, not just trusted from the pipeline's own report — the first time this specific verification has fully passed in this project's history. Directly confirmed (not merely theorized) that the fix does **not** survive a reboot: after a clean `reboot`, `ip -4 addr show vmbr0` and `ip -4 route show` both return nothing — `ifupdown2`'s boot dispatch still only processes the first `iface vmbr0` stanza, and the supplementary `dhclient` call only covers the apply-time invocation. Also found: SLIRP's DHCP router-option behavior (decision record 15's finding) is **inconsistent across sessions** on this same host — this pass's SLIRP server sent a `routers` option where decision record 15's did not, with no code change between them — not resolved to a single explanation. See [decision-records/12-gateA-additive-repair.md](decision-records/12-gateA-additive-repair.md) through [17-gateA-apply-time-pass-reboot-gap-confirmed.md](decision-records/17-gateA-apply-time-pass-reboot-gap-confirmed.md). 81/81 unit tests passing. **Gate A still does not fully pass** — reboot-persistence, one of its own stated criteria, fails, now with direct evidence rather than a theoretical gap. Next concrete step: a boot-time systemd unit re-running `dhclient` for additive-repair-created `inet dhcp` stanzas that ifupdown2's own dispatch doesn't bring up, scoped narrowly (not attempted this pass). The guestfwd infrastructure problem (decision record 16) remains separately unresolved and still blocks the automated/answer-file install path specifically; it no longer blocks Gate A verification itself, since this pass found a working route around it. **No push to `cliffthelin/baseline` at any point.** Gates B–F remain unstarted.
